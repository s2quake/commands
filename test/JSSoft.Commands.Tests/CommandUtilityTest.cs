...
public class CommandUtilityTest(ITestOutputHelper output)
{
    ...
    public void SplitCommandLine_Test(
        string commandLine, string expectedCommandName, string[] expectedCommandArguments)
    {
        var (actualCommandName, actualCommandArguments)
            = CommandUtility.SplitCommandLine(commandLine);
        Assert.Equal(expectedCommandName, actualCommandName);
        Assert.Equivalent(expectedCommandArguments, actualCommandArguments);
        Assert.True(CommandUtility.TrySplitCommandLine(commandLine, out var v1, out var v2));
    }
    ...
    public void SplitCommandLine_FailTest(string commandLine)
    {
        Assert.Throws<ArgumentException>(
            nameof(commandLine), () => CommandUtility.SplitCommandLine(commandLine));
        Assert.False(CommandUtility.TrySplitCommandLine(commandLine, out var v1, out var v2));
    }
